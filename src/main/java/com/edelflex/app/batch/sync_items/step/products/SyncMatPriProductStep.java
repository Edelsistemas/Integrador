...
public class SyncMatPriProductStep {

  @Bean("syncMatPriProductStepDefinition")
  public Step syncMatPriProductStep(
      StepBuilderFactory stepBuilderFactory,
      @Qualifier("matPriReader") SyncBaseProductReader<MatPriProduct> reader,
      @Qualifier("matPriProcessor") SyncBaseProductProcessor<MatPriProduct> processor,
      @Qualifier("matPriWriter") SyncBaseProductWriter writer) {
    return stepBuilderFactory
        .get("Sync MatPri Step")
        .<MatPriProduct, ProductProcessInfo>chunk(100)
        .reader(reader)
        .processor(processor)
        .writer(writer)
        .faultTolerant()
  ...
  @Bean("matPriReader")
  public SyncBaseProductReader<MatPriProduct> reader(
      @Qualifier("jdbcTemplateSQLServer") JdbcTemplate jdbcTemplate,
      @Value("${team-center.querys.mat-pri.get}") String query) {
    return new SyncBaseProductReader<>(jdbcTemplate, query, MatPriProduct.class);
  }

  @Bean("matPriProcessor")
  public SyncBaseProductProcessor<MatPriProduct> processor(SapItemService sapItemService) {
  ...
  public SyncBaseProductWriter writer(
      ...
      @Qualifier("jdbcTemplateSQLServer") JdbcTemplate jdbcTemplate,
      @Value("${team-center.querys.mat-pri.update}") String updateQuery) {
