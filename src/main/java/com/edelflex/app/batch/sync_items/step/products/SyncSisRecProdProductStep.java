...
public class SyncSisRecProdProductStep {

  @Bean("syncSisRecProdProductStepDefinition")
  public Step syncSisRecProdProductStep(
      StepBuilderFactory stepBuilderFactory,
      @Qualifier("sisRecProdReader") SyncBaseProductReader<SisRecProdProduct> reader,
      @Qualifier("sisRecProdProcessor") SyncBaseProductProcessor<SisRecProdProduct> processor,
      @Qualifier("sisRecProdWriter") SyncBaseProductWriter writer) {
    return stepBuilderFactory
        .get("Sync SisRecProd Step")
        .<SisRecProdProduct, ProductProcessInfo>chunk(100)
        .reader(reader)
        .processor(processor)
        .writer(writer)
        .faultTolerant()
  ...
  @Bean("sisRecProdReader")
  public SyncBaseProductReader<SisRecProdProduct> reader(
      @Qualifier("jdbcTemplateSQLServer") JdbcTemplate jdbcTemplate,
      @Value("${team-center.querys.sis-rec-prod.get}") String query) {
    return new SyncBaseProductReader<>(jdbcTemplate, query, SisRecProdProduct.class);
  }

  @Bean("sisRecProdProcessor")
  public SyncBaseProductProcessor<SisRecProdProduct> processor(SapItemService sapItemService) {
    return new SyncBaseProductProcessor<>(sapItemService);
  }

  @Bean("sisRecProdWriter")
  public SyncBaseProductWriter writer(
      MongoTemplate mongoTemplate,
      @Qualifier("jdbcTemplateSQLServer") JdbcTemplate jdbcTemplate,
      @Value("${team-center.querys.sis-rec-prod.update}") String updateQuery) {
