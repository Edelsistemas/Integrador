...
public class SyncValRetenProductStep {

  @Bean("syncValRetenProductStepDefinition")
  public Step syncValRetenProductStep(
          StepBuilderFactory stepBuilderFactory,
          @Qualifier("valRetenReader") SyncBaseProductReader<ValRetenProduct> reader,
          @Qualifier("valRetenProcessor") SyncBaseProductProcessor<ValRetenProduct> processor,
          @Qualifier("valRetenWriter") SyncBaseProductWriter writer) {
    return stepBuilderFactory
        .get("Sync ValReten Step")
        .<ValRetenProduct, ProductProcessInfo>chunk(100)
        .reader(reader)
        .processor(processor)
        .writer(writer)
        .faultTolerant()
  ...
  @Bean("valRetenReader")
  public SyncBaseProductReader<ValRetenProduct> reader(
      @Qualifier("jdbcTemplateSQLServer") JdbcTemplate jdbcTemplate,
      @Value("${team-center.querys.val-reten.get}") String query) {
    return new SyncBaseProductReader<>(jdbcTemplate, query, ValRetenProduct.class);
  }

  @Bean("valRetenProcessor")
  public SyncBaseProductProcessor<ValRetenProduct> processor(SapItemService sapItemService) {
